...
local function isNavigatedDown()
	return IsControlJustPressed(0, keys.down) or IsControlJustReleased(2, keys.scrollDown)
end

local function isNavigatedUp()
	return IsControlJustPressed(0, keys.up) or IsControlJustReleased(2, keys.scrollUp)
end
...
function WarMenu.Display()
	...
	if not IsPauseMenuActive() then
		...
		if skipInputNextFrame then
			...
			if IsControlPressed(0, keys.down) and not keyDelay then
				keyDelay = true
				SetTimeout(150, function() keyDelay = false end)
				PlaySoundFrontend(-1, 'NAV_UP_DOWN', 'HUD_FRONTEND_DEFAULT_SOUNDSET', true)
			...
			elseif IsControlPressed(0, keys.up) and not keyDelay then
				keyDelay = true
				SetTimeout(150, function() keyDelay = false end)
				PlaySoundFrontend(-1, 'NAV_UP_DOWN', 'HUD_FRONTEND_DEFAULT_SOUNDSET', true)
			...
			elseif IsControlPressed(2, keys.left) and not keyDelay then
				keyDelay = true
				SetTimeout(100, function() keyDelay = false end)
				currentKey = keys.left
			elseif IsControlPressed(2, keys.right) and not keyDelay then
				keyDelay = true
				SetTimeout(100, function() keyDelay = false end)
